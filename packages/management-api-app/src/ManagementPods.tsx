...
import { MPodsByProject, ManagedPod } from '@hawtio/online-management-api'
import {
  DescriptionList,
  ...
  return (
    <Panel isScrollable>
      <PanelMain>
        <PanelMainBody>
          {
            Object.entries(props.podsByProject).map(([project, podsByUid]) => (
              <TableComposable key={project} aria-label='Pods table' variant='compact'>
                <Caption>Pods in Project {project}</Caption>
                <Thead>
                  <Tr>
                    <Th>Name</Th>
                    <Th>Namespace</Th>
                ...
                <Tbody>
                  {Object.values(podsByUid).map(pod => (
